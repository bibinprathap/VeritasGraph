...
<img src="VeritasReason Logo.png" alt="VeritasReason Logo" width="420"/>
...
Ask **"Which purchase orders violated our Segregation-of-Duties policy in Q1?"** —
the engine pulls the policy clause via GraphRAG, runs the YAML rules over your
ERP triples, and returns the violators **with the rule ID and the exact policy
citation**. Try it yourself:
...
> Works alongside **Agno** and any LLM — VeritasReason is the **accountability layer** on top, not a replacement. LangChain, LangGraph, CrewAI, and more coming soon.
...
<sub>First-class · <code>pip install veritas-reason[agno]</code></sub>
</td>
</tr>
<tr>
...
> **Agno — First-Class Integration** · `pip install veritas-reason[agno]`
>
...
VeritasReason ships a full **MCP server** (`veritasreason/mcp_server.py`) — run it once and any MCP-compatible tool connects automatically:
...
## 🖥️ VeritasReason Knowledge Explorer
...
- **SHACL shape generation** — `OntologyEngine.to_shacl()` auto-derives SHACL node and property shapes from any VeritasReason ontology dict; zero hand-authoring; deterministic (same ontology → same shapes)
...
| `veritasreason.graph_store` | Graph database backends — Neo4j, FalkorDB, Apache AGE, Amazon Neptune; Cypher queries |
| `veritasreason.embeddings` | Text embedding generation — Sentence-Transformers, FastEmbed, OpenAI, BGE; similarity calculation |
...
| `veritasreason.parse` | Document parsing — PDF, DOCX, PPTX, HTML, code, email, structured data, media with OCR |
| `veritasreason.split` | Document chunking — recursive, semantic, entity-aware, relation-aware, graph-based, ontology-aware |
...
| `veritasreason.triplet_store` | RDF triplet store integration — Blazegraph, Jena, RDF4J; SPARQL queries and bulk loading |
...
| [`explorer/`](explorer/) | **VeritasReason Knowledge Explorer** — React 19 + Sigma.js UI: graph canvas, decision viewer, causal chains, entity resolution, ontology browser, and registry audit log |
...
| `veritasreason.llms` | LLM provider integrations — Groq, OpenAI, Novita AI, HuggingFace, LiteLLM |
| `veritasreason.utils` | Shared utilities — logging, validation, exception handling, constants, types, progress tracking |
...
- **`veritasreason.context`** — context graphs, decisions, causal chains, precedent search, policy engine, checkpoints
- **`veritasreason.kg`** — KG construction, graph algorithms, embeddings, link prediction, temporal query engine, Allen algebra, `TemporalNormalizer`, provenance
- **`veritasreason.semantic_extract`** — NER, relation extraction, triplet generation, LLM extraction with temporal bounds, deduplication
- **`veritasreason.reasoning`** — forward chaining, Rete, deductive, abductive, SPARQL, temporal algebra
- **`veritasreason.vector_store`** — FAISS, Pinecone, Weaviate, Qdrant, Milvus, PgVector, in-memory; hybrid & filtered search
- **`veritasreason.export`** — RDF (Turtle/JSON-LD/N-Triples/XML), OWL-Time, Parquet, ArangoDB AQL, OWL, SHACL
- **`veritasreason.ingest`** — files, web crawl, databases, Snowflake, email, repositories
- **`veritasreason.ontology`** — OWL generation & import, SHACL generation & validation, SKOS vocabulary management
- **`veritasreason.pipeline`** — stage chaining, parallel workers, validation, retry policies, failure handling
- **`veritasreason.graph_store`** — Neo4j, FalkorDB, Apache AGE, Amazon Neptune; Cypher queries
- **`veritasreason.embeddings`** — Sentence-Transformers, FastEmbed, OpenAI, BGE; similarity
- **`veritasreason.deduplication`** — entity dedup, similarity scoring, blocking and semantic strategies
- **`veritasreason.provenance`** — W3C PROV-O lineage, revision history, audit log export
- **`veritasreason.parse`** — PDF, DOCX, PPTX, HTML, code, email, media with OCR
- **`veritasreason.split`** — recursive, semantic, entity-aware, graph-based, ontology-aware chunking
- **`veritasreason.conflicts`** — multi-source conflict detection with resolution strategies
- **`veritasreason.change_management`** — version storage, checksums, audit trails, compliance support
- **`veritasreason.triplet_store`** — Blazegraph, Jena, RDF4J; SPARQL, bulk loading, SKOS helpers
- **`veritasreason.visualization`** — KG, ontology, embedding, and temporal graph visualization
- **`veritasreason.llms`** — Groq, OpenAI, Novita AI, HuggingFace, LiteLLM
- **[`explorer/`](explorer/)** — **VeritasReason Knowledge Explorer** — browser UI for live graph inspection, decisions, entity resolution, and ontology browsing (`npm run dev` in `explorer/`)

### Graph Databases
- **Neo4j** — Cypher queries via `veritasreason.graph_store`
- **FalkorDB** — native support; `DecisionQuery` and `CausalChainAnalyzer` work directly with FalkorDB row/header shapes
...
- **Pinecone** — `pip install veritas-reason[vectorstore-pinecone]`
- **Weaviate** — `pip install veritas-reason[vectorstore-weaviate]`
- **Qdrant** — `pip install veritas-reason[vectorstore-qdrant]`
- **Milvus** — `pip install veritas-reason[vectorstore-milvus]`
- **PgVector** — `pip install veritas-reason[vectorstore-pgvector]`

### Data Sources
- **Files** — PDF, DOCX, HTML, JSON, CSV, Excel, PPTX, archives
- **Web** — configurable-depth crawler
- **Databases** — SQL via `DBIngestor`
- **Snowflake** — table/query ingestion, pagination, password/key-pair/OAuth/SSO auth · `pip install veritas-reason[db-snowflake]`
