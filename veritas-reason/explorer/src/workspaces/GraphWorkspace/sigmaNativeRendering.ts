...
type VeritasReasonNodeDrawData = {
  x: number;
  y: number;
  size: number;
  label: string;
...
class EntityTokenNodeProgram extends NodeProgram<(typeof ENTITY_TOKEN_UNIFORMS)[number]> {
  ...
  drawLabel = drawVeritasReasonNodeLabel;

  drawHover = drawVeritasReasonNodeHover;
  ...
  processVisibleItem(nodeIndex: number, startIndex: number, data: NodeDisplayData & VeritasReasonNodeDrawData): void {
...
export const drawVeritasReasonNodeLabel: NodeLabelDrawingFunction = (context, rawData) => {
  const data = rawData as typeof rawData & VeritasReasonNodeDrawData;
  if (!data.label) {
    return;
  }
...
export const drawVeritasReasonNodeHover: NodeHoverDrawingFunction = (context, rawData) => {
  const data = rawData as typeof rawData & VeritasReasonNodeDrawData;
  if (!data.label) {
    return;
  }
...
export const VERITASREASON_NODE_PROGRAM_CLASSES = {
  ...DEFAULT_NODE_PROGRAM_CLASSES,
  circle: EntityTokenNodeProgram,
};

export const VERITASREASON_EDGE_PROGRAM_CLASSES = {
