...
| npm | 9.x or higher |
| Python | 3.8+ |
| VeritasReason backend | running on `http://127.0.0.1:8000` |
...
### 2. Install the VeritasReason Python package
...
### 3. Start the VeritasReason backend
...
├── index.html
├── vite.config.ts                     # Dev proxy → 127.0.0.1:8000, build → ../veritasreason/static
└── package.json
```
...
# Type-check and build a production bundle into ../veritasreason/static
npm run build
...
The compiled assets are written to `../veritasreason/static/`. The VeritasReason Python server serves this folder automatically at its root URL — no separate web server needed.
...
- Make sure the VeritasReason backend is running (`python -m veritasreason.server`) before opening the UI.
