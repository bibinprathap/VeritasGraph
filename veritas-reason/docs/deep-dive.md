...
The full data flow through a VeritasReason pipeline:
...
sequenceDiagram
    ...
    User->>VeritasReason: build_knowledge_base(sources)
    VeritasReason->>Ingestor: ingest(sources)
...
VeritasReason's reasoning engines can derive implicit relationships:
...
def process_large_dataset(sources, batch_size=100):
    for i in range(0, len(sources), batch_size):
        ...
        result = veritasreason.build_knowledge_base(batch)
        save_result(result)
        del result
        gc.collect()
```
...
```python
from veritasreason.core import Plugin
...
```python
from veritasreason.semantic_extract import BaseExtractor
...
```python
from veritasreason.ingest import BaseIngestor
...
| API | Purpose |
|-----|---------|
| `VeritasReason.build_knowledge_base()` | Main orchestration entry point |
