...
The VeritasReason change management module provides enterprise-grade version control, audit trails, and compliance tracking for knowledge graphs and ontologies. Designed for high-stakes domains where every change must be tracked, verified, and auditable with complete data integrity guarantees.
...
```python
from veritasreason.change_management import TemporalVersionManager
...
```python
from veritasreason.change_management import ChangeLogEntry
...
```python
from veritasreason.change_management import InMemoryVersionStorage
...
```python
from veritasreason.change_management import SQLiteVersionStorage
...
```python
from veritasreason.change_management import BaseVersionManager
...
```python
from veritasreason.change_management import OntologyVersionManager
...
VeritasReason supports **Delta-Aware Pipelines**, allowing you to compute the exact differences (added and removed triples)
...
```python
from veritasreason.change_management import TemporalVersionManager
from veritasreason.pipeline import PipelineBuilder, ExecutionEngine
...
```python
from veritasreason.change_management import compute_checksum
...
```python
from veritasreason.change_management import verify_checksum
...
```python
from veritasreason.utils.exceptions import ValidationError, ProcessingError
...
VeritasReason allows you to treat ontology schema changes with the same rigor as database migrations. By comparing two versions, you can generate a machine-readable diff and a structured impact report to catch breaking changes before they reach production.
