...
The VeritasReason provenance module provides W3C PROV-O compliant tracking for knowledge graphs, enabling complete end-to-end lineage from source documents to query responses. Designed for high-stakes domains where every decision must be explainable and auditable.
...
-   :material-all-inclusive:{ .lg .middle } **Complete Coverage**
    ...
    All 17 VeritasReason modules integrated for comprehensive tracking
...
- ✅ **All 17 Modules Integrated** — Complete coverage across VeritasReason
...
```python
from veritasreason.provenance import ProvenanceEntry
...
All VeritasReason modules have provenance-enabled versions. Enable tracking by setting `provenance=True`.
...
```python
from veritasreason.provenance.bridge_axiom import BridgeAxiom, TranslationChain
