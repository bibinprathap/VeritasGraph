<div align="center">
  <img src="assets/img/VeritasReason Logo.png" alt="VeritasReason Logo" width="420" height="auto">
...
VeritasReason is the **context and intelligence layer** you add to your AI stack:
...
Works alongside LangChain, LlamaIndex, AutoGen, CrewAI, and any LLM provider — VeritasReason is not a replacement, it's the accountability layer on top.
...
| `veritasreason.kg` | KG construction, graph algorithms, centrality, community detection, embeddings, link prediction |
| `veritasreason.semantic_extract` | NER, relation extraction, event extraction, coreference, triplet generation, LLM extraction |
| `veritasreason.reasoning` | Forward chaining, Rete network, deductive, abductive, SPARQL reasoning, explanation generation |
| `veritasreason.vector_store` | FAISS, Pinecone, Weaviate, Qdrant, Milvus, PgVector; hybrid & filtered search |
| `veritasreason.export` | RDF, Parquet, ArangoDB AQL, CSV, YAML, OWL, graph formats |
| `veritasreason.ingest` | Files, web crawl, feeds, databases, Snowflake, MCP, email, repositories |
| `veritasreason.ontology` | Auto-generation, OWL/RDF export, import, validation, versioning |
| `veritasreason.pipeline` | Pipeline DSL, parallel workers, validation, retry policies, failure handling |
| `veritasreason.graph_store` | Neo4j, FalkorDB, Apache AGE, Amazon Neptune; Cypher queries |
| `veritasreason.embeddings` | Sentence-Transformers, FastEmbed, OpenAI, BGE; similarity calculation |
| `veritasreason.deduplication` | Entity deduplication, similarity scoring, merging, clustering |
| `veritasreason.provenance` | W3C PROV-O lineage tracking, source attribution, audit trails |
| `veritasreason.parse` | PDF, DOCX, PPTX, HTML, code, email, structured data, OCR |
| `veritasreason.split` | Recursive, semantic, entity-aware, relation-aware, graph-based chunking |
| `veritasreason.normalize` | Text, entities, dates, numbers, quantities, languages, encodings |
| `veritasreason.conflicts` | Multi-source conflict detection (value, type, temporal, logical) with resolution |
| `veritasreason.change_management` | Version storage, change tracking, checksums, audit trails |
| `veritasreason.triplet_store` | Blazegraph, Jena, RDF4J; SPARQL queries and bulk loading |
| `veritasreason.visualization` | Interactive/static KG, ontology, embedding, and temporal graph visualization |
| `veritasreason.core` | Framework orchestration, configuration, plugin system |
| `veritasreason.llms` | Groq, OpenAI, Novita AI, HuggingFace, LiteLLM integrations |
...
=== "Docker"
    ...
    ```bash
    docker pull veritasreason/veritasreason:latest
    docker run -it veritasreason/veritasreason
    ```
