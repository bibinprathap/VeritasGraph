...
PostgreSQL with pgvector extension support for VeritasReason vector storage and similarity search.
...
```python
# Local development
"postgresql://postgres:postgres@localhost:5432/veritasreason"

# With SSL
"postgresql://user:pass@host/db?sslmode=require"

# Connection parameters
"postgresql://user:pass@localhost/db?connect_timeout=10&application_name=veritasreason"
```
...
```python
from veritasreason.vector_store import PgVectorStore
import numpy as np

# Initialize store
store = PgVectorStore(
    connection_string="postgresql://postgres:postgres@localhost:5432/veritasreason",
...
```bash
# Backup
pg_dump -h localhost -U postgres -d veritasreason -t document_vectors > vectors_backup.sql

# Restore
psql -h localhost -U postgres -d veritasreason < vectors_backup.sql
```
...
export VERITASREASON_PGVECTOR_URL="postgresql://user:pass@host/db"
```
...
store = PgVectorStore(
    connection_string=os.getenv("VERITASREASON_PGVECTOR_URL"),
    table_name="vectors",
    dimension=768,
    distance_metric="cosine"
)
