...
Apache AGE is a PostgreSQL extension that adds graph database functionality, enabling you to run openCypher queries alongside traditional SQL. This backend lets VeritasReason use AGE as a property graph store with the same interface as Neo4j and FalkorDB.
...
node = store.create_node(["Entity"], {"veritasreason_id": "ent-001", "value": "test"})
print(node)
# {"id": 844424930131969, "labels": ["Entity"], "properties": {"veritasreason_id": "ent-001", "value": "test"}}
...
| `GRAPH_STORE_AGE_GRAPH_NAME` | AGE graph name | `veritasreason` |
...
| **Semantic ID** | Application-level identifier. Store it in the `veritasreason_id` property. |

```python
node = store.create_node(
    labels=["Document"],
    properties={"veritasreason_id": "doc-abc-123", "title": "My Doc"},
)
# node["id"] → AGE internal ID (e.g., 844424930131969)
# node["properties"]["veritasreason_id"] → "doc-abc-123"
```

> **Important**: Never mix AGE internal IDs with semantic IDs. Use `node["id"]` for graph operations (delete, update, traverse) and `node["properties"]["veritasreason_id"]` for application-level lookups.

---

## Label Handling

AGE supports exactly **one label per vertex**. VeritasReason handles this transparently:
...
All methods match the standard VeritasReason graph store backend interface:
