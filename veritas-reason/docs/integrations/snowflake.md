...
VeritasReason features a native integration with **Snowflake**, the powerful cloud data warehouse that enables scalable data storage and analytics for enterprise workloads.
...
*   **Document Export**: Convert Snowflake data to VeritasReason document format.

### Basic Usage

```python
from veritasreason.ingest import SnowflakeIngestor
...
# 6. Export as documents for VeritasReason processing
...
Snowflake connector is an optional dependency for VeritasReason. You can install it along with VeritasReason or as a separate requirement.

### Install via VeritasReason
```bash
# Install with Snowflake support
pip install veritas-reason[db-snowflake]

# Or install with all database connectors
pip install veritas-reason[db-all]
```
...
- **[Getting Started Guide](../getting-started.md)** - Quick start with VeritasReason
