...
Every VeritasReason module works independently — use only what you need.
...
VeritasReason is organized into **six logical layers** - each with specific responsibilities:
...
```python
from veritasreason.normalize import DataNormalizer
...
```python
from veritasreason.kg import GraphBuilder, GraphAnalyzer
...
```python
from veritasreason.reasoning import ReasoningEngine
...
```python
from veritasreason.deduplication import EntityResolver
...
```python
from veritasreason.context import ContextManager
...
```python
from veritasreason.seed import SeedData
...
```python
from veritasreason.llms import LLMProvider
...
```python
from veritasreason.visualization import GraphVisualizer
...
```python
from veritasreason.pipeline import Pipeline
...
result = veritasreason.process("data/")
```
...
| [Core](reference/core.md) | Framework orchestration | VeritasReason, Config | System management |
