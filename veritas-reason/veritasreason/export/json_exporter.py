...
VeritasReason framework, enabling structured data export for knowledge graphs and
...
class JSONExporter:
    ...
    def export_entities(
        ...
        json_data = {
            "@context": {
                ...
                "entities": {"@id": "veritasreason:entities", "@container": "@list"},
            },
            "entities": entities,
            "metadata": {
        ...
        **options,
        ...
        json_data = {
            "@context": {
                "@vocab": "https://veritasreason.dev/vocab/",
                "relationships": {
                    "@id": "veritasreason:relationships",
                    "@container": "@list",
                },
            },
    ...
    def _convert_to_jsonld(
        ...
        **options,
        ...
        if include_metadata:
            jsonld["@id"] = f"https://veritasreason.dev/data/{datetime.now().isoformat()}"
            if include_provenance:
                jsonld["veritasreason:exportedAt"] = datetime.now().isoformat()
                jsonld["veritasreason:format"] = "json-ld"
    ...
    def _convert_kg_to_jsonld(self, kg: Dict[str, Any], **options) -> Dict[str, Any]:
        ...
        jsonld = {
            ...
            "@id": f"https://veritasreason.dev/graph/{datetime.now().isoformat()}",
            "@type": "veritasreason:KnowledgeGraph",
        }

        # Convert entities to JSON-LD format
        entities = kg.get("entities", [])
        if entities:
            jsonld["veritasreason:entities"] = [self._entity_to_jsonld(e) for e in entities]
            self.logger.debug(f"Converted {len(entities)} entity(ies) to JSON-LD")

        # Convert relationships to JSON-LD format
        relationships = kg.get("relationships", [])
        if relationships:
            jsonld["veritasreason:relationships"] = [
        ...
        jsonld["veritasreason:exportedAt"] = datetime.now().isoformat()
        if "metadata" in kg:
            jsonld["veritasreason:metadata"] = kg["metadata"]
    ...
    def _entity_to_jsonld(self, entity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        jsonld = {
            "@id": entity_id,
            "@type": entity.get("type") or "veritasreason:Entity",
        ...
        if "metadata" in entity:
            jsonld["veritasreason:metadata"] = entity["metadata"]
    ...
    def _relationship_to_jsonld(self, rel: Dict[str, Any]) -> Dict[str, Any]:
        ...
        jsonld = {
            ...
            "veritasreason:source": {"@id": rel.get("source_id") or rel.get("source")},
            "veritasreason:target": {"@id": rel.get("target_id") or rel.get("target")},
            "veritasreason:confidence": rel.get("confidence", 1.0),
        }

        # Add metadata if present
        if "metadata" in rel:
            jsonld["veritasreason:metadata"] = rel["metadata"]
