...
capabilities for the VeritasReason framework, supporting multiple RDF serialization
...
class NamespaceManager:
    ...
    def __init__(self, **config):
        ...
        self.namespaces: Dict[str, str] = {
            ...
            "veritasreason": "https://veritasreason.dev/ns#",
        }
...
class RDFSerializer:
    ...
    # veritasreason:openEndedInterval "true"^^xsd:boolean on the time:Interval
    ...
    def serialize_to_turtle(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        for entity in entities:
            ...
            if not entity_id:
                ...
                entity_id = f"veritasreason:entity_{hash(entity_text)}"

            entity_type = entity.get("type", "veritasreason:Entity")
            text = entity.get("text") or entity.get("label", "")
            confidence = entity.get("confidence", 1.0)

            lines.append(f"<{entity_id}> a <{entity_type}> ;")
            lines.append(f'    veritasreason:text "{text}" ;')
            lines.append(f"    veritasreason:confidence {confidence} .")
            lines.append("")
        ...
        for idx, rel in enumerate(relationships):
            ...
            rel_type = rel.get("type", "veritasreason:related_to")
    ...
    def _owl_time_triples_for_rel(
        ...
        For TemporalBound.OPEN valid_until values we use:
            veritasreason:openEndedInterval "true"^^xsd:boolean
        ...
        rel_base_id = (
            rel.get("id")
            or f"veritasreason:rel_{idx}_{hash(str(rel.get('source_id', '')) + str(rel.get('target_id', '')))}"
        )
    ...
    def serialize_to_rdfxml(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        lines.append('         xmlns:veritasreason="https://veritasreason.dev/ns#">')
        lines.append("")
        ...
        for entity in entities:
            ...
            lines.append(f"    <veritasreason:text>{text}</veritasreason:text>")
            lines.append(
                f"    <veritasreason:confidence>{confidence}</veritasreason:confidence>"
            )
    ...
    def serialize_to_jsonld(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        jsonld = {
            "@context": {
                "@vocab": "https://veritasreason.dev/vocab/",
                "veritasreason": "https://veritasreason.dev/ns#",
                "rdf": "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
                "rdfs": "http://www.w3.org/2000/01/rdf-schema#",
            },
        ...
        for entity in entities:
            ...
            if not entity_id:
                entity_text = entity.get("text", "")
                entity_id = f"veritasreason:entity/{entity_text}"

            jsonld["@graph"].append(
                {
                    "@id": entity_id,
                    "@type": entity.get("type", "veritasreason:Entity"),
                    "veritasreason:text": entity.get("text") or entity.get("label", ""),
                    "veritasreason:confidence": entity.get("confidence", 1.0),
                }
            )
        ...
        for rel in relationships:
            ...
            if not rel_id:
                ...
                rel_id = f"veritasreason:rel/{source_id}_{target_id}"

            jsonld["@graph"].append(
                {
                    "@id": rel_id,
                    "@type": "veritasreason:Relationship",
                    "veritasreason:source": {
                        "@id": rel.get("source_id") or rel.get("source")
                    },
                    "veritasreason:target": {
                        "@id": rel.get("target_id") or rel.get("target")
                    },
                    "veritasreason:type": rel.get("type", "related_to"),
                }
            )
    ...
    def serialize_to_ntriples(self, rdf_data: Dict[str, Any], **options) -> str:
        ...
        def expand_uri(uri: str) -> str:
            ...
            if uri.startswith("http"):
                return f"<{uri}>"
            if uri.startswith("veritasreason:"):
                return f"<https://veritasreason.dev/ns#{uri.split(':', 1)[1]}>"
            ...
            if ":" in uri:
                return f"<{uri}>"
            return f"<https://veritasreason.dev/ns#{uri}>"
        ...
        for entity in entities:
            ...
            if text:
                ...
                lines.append(
                    f'{subject} {expand_uri("veritasreason:text")} "{safe_text}" .'
                )
            ...
            if confidence is not None:
                lines.append(
                    f'{subject} {expand_uri("veritasreason:confidence")} "{confidence}"^^<http://www.w3.org/2001/XMLSchema#float> .'
                )
