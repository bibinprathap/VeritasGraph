...
Example Usage:
    >>> from veritasreason.utils import track_progress
    >>> 
    >>> @track_progress
...
DISABLE_JUPYTER_PROGRESS = os.getenv("VERITASREASON_DISABLE_JUPYTER_PROGRESS", "").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
...
class ConsoleProgressDisplay(ProgressDisplay):
    ...
    def _get_action_message(self, module: Optional[str], message: str) -> str:
        ...
        base_msg = f"VeritasReason {action}"

        if not message:
            return base_msg
            
        # If message already contains VeritasReason format, use it
        if "VeritasReason:" in message:
            return message.split("VeritasReason:")[-1].strip()
        ...
        with self.lock:
            ...
            if pipeline_items:
                ...
                if not hasattr(self, '_pipeline_header_shown'):
                    if self.use_emoji:
                        sys.stdout.write("🧠 VeritasReason - 📊 Current Progress\n")
                    else:
                        sys.stdout.write("VeritasReason - Current Progress\n")
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        with self.lock:
            ...
            # Summary header
            if self.use_emoji:
                self._safe_write("🧠 VeritasReason - 📊 Progress Summary\n")
            else:
                self._safe_write("VeritasReason - Progress Summary\n")
            self._safe_write("=" * 80 + "\n")
...
class JupyterProgressDisplay(ProgressDisplay):
    ...
    def _get_action_message(self, module: Optional[str], message: str) -> str:
        """Get action message based on module."""
        # Extract action from message if it contains "VeritasReason:"
        if message and "VeritasReason:" in message:
            # Use the message as-is if it already has VeritasReason format
            return (
                message.split("VeritasReason:")[-1].strip()
                if "VeritasReason:" in message
                else message
            )
    ...
    def _build_html(self, items: List[ProgressItem]) -> str:
        ...
        # Current status
        html_parts.append("<h4>🧠 VeritasReason - 📊 Current Progress</h4>")
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        html_parts = ["<div style='font-family: monospace;'>"]
        html_parts.append("<h3>🧠 VeritasReason - 📊 Progress Summary</h3>")
...
class FileProgressDisplay(ProgressDisplay):
    ...
    def show_summary(self, items: List[ProgressItem]) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info("VERITASREASON - PROGRESS SUMMARY")
        self.logger.info("=" * 80)
...
class ModuleDetector:
    ...
    def detect_from_frame(frame) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Extract module name (e.g., 'veritasreason.ingest.file_ingestor' -> 'ingest')
            if "veritasreason." in module_name:
