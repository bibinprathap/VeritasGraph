...
This module provides comprehensive logging utilities for the VeritasReason framework,
...
Example Usage:
    >>> from veritasreason.utils import setup_logging, get_logger
    >>> logger = setup_logging(level="INFO", file="app.log")
    >>> module_logger = get_logger(__name__)
    >>> module_logger.info("Processing started")
    >>> 
    >>> from veritasreason.utils import log_performance, log_error
    ...
    >>> 
    >>> from veritasreason.utils import log_execution_time
    >>> @log_execution_time
    ... def my_function():
...
def setup_logging(config: Optional[Dict[str, Any]] = None, **kwargs) -> logging.Logger:
    """
    Setup logging configuration for VeritasReason framework.
    ...
    log_file = config.get("file", "veritasreason.log")
    ...
    root_logger = logging.getLogger("veritasreason")
...
def get_logger(
    ...
    Returns:
        Logger instance
    """
    logger = logging.getLogger(f"veritasreason.{name}")
