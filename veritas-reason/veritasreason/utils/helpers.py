...
VeritasReason framework, providing common data manipulation, file handling, string
...
Example Usage:
    >>> from veritasreason.utils import clean_text, normalize_entities
    >>> cleaned = clean_text("  Hello   World  ")
    >>> entities = normalize_entities([{"id": "e1", "text": "John", "type": "PERSON"}])
    >>> 
    >>> from veritasreason.utils import hash_data, safe_filename
    >>> data_hash = hash_data({"key": "value"})
    >>> safe_name = safe_filename("my file.txt")
    >>> 
    >>> from veritasreason.utils import merge_dicts, get_nested_value
    >>> merged = merge_dicts({"a": 1}, {"b": 2}, deep=True)
    >>> value = get_nested_value(config, "database.host", default="localhost")
    >>> 
    >>> from veritasreason.utils import retry_on_error
