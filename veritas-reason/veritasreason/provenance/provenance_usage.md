...
The VeritasReason provenance module provides W3C PROV-O compliant tracking for knowledge graphs, enabling complete end-to-end lineage from source documents to query responses. This module consolidates and enhances provenance tracking from:
...
The provenance module is included with VeritasReason. No additional installation required.

```python
from veritasreason.provenance import ProvenanceManager
```
...
- **prov:Agent** - Who/what performed the activity (VeritasReason, user, model)
...
```python
from veritasreason.provenance import SourceReference
...
All VeritasReason modules now have provenance-enabled versions. Simply add `provenance=True` to enable tracking.

### Semantic Extract (NER, Relations, Events)

```python
from veritasreason.semantic_extract.semantic_extract_provenance import (
...
```python
from veritasreason.llms.llms_provenance import (
...
```python
from veritasreason.pipeline.pipeline_provenance import PipelineWithProvenance
...
```python
from veritasreason.context.context_provenance import ContextManagerWithProvenance
...
```python
from veritasreason.ingest.ingest_provenance import PDFIngestorWithProvenance
...
```python
from veritasreason.embeddings.embeddings_provenance import EmbeddingGeneratorWithProvenance
...
```python
from veritasreason.graph_store.graph_store_provenance import GraphStoreWithProvenance
...
```python
from veritasreason.vector_store.vector_store_provenance import VectorStoreWithProvenance
...
```python
from veritasreason.triplet_store.triplet_store_provenance import TripletStoreWithProvenance
...
```python
# Reasoning
from veritasreason.reasoning.reasoning_provenance import ReasoningEngineWithProvenance
reasoner = ReasoningEngineWithProvenance(provenance=True)

# Conflicts
from veritasreason.conflicts.conflicts_provenance import SourceTrackerWithUnifiedBackend
tracker = SourceTrackerWithUnifiedBackend()

# Deduplication
from veritasreason.deduplication.deduplication_provenance import DeduplicatorWithProvenance
dedup = DeduplicatorWithProvenance(provenance=True)

# Export
from veritasreason.export.export_provenance import ExporterWithProvenance
exporter = ExporterWithProvenance(provenance=True)

# Parse
from veritasreason.parse.parse_provenance import ParserWithProvenance
parser = ParserWithProvenance(provenance=True)

# Normalize
from veritasreason.normalize.normalize_provenance import NormalizerWithProvenance
normalizer = NormalizerWithProvenance(provenance=True)

# Ontology
from veritasreason.ontology.ontology_provenance import OntologyManagerWithProvenance
ontology = OntologyManagerWithProvenance(provenance=True)

# Visualization
from veritasreason.visualization.visualization_provenance import VisualizerWithProvenance
...
from veritasreason.provenance import ProvenanceManager
from veritasreason.ingest.ingest_provenance import PDFIngestorWithProvenance
from veritasreason.semantic_extract.semantic_extract_provenance import NERExtractorWithProvenance
from veritasreason.llms.llms_provenance import GroqLLMWithProvenance
from veritasreason.graph_store.graph_store_provenance import GraphStoreWithProvenance
...
```python
from veritasreason.provenance import ProvenanceManager
from veritasreason.provenance.bridge_axiom import BridgeAxiom
...
```python
from veritasreason.provenance.bridge_axiom import create_translation_chain, BridgeAxiom
...
from veritasreason.provenance.bridge_axiom import trace_translation_chain
...
```python
from veritasreason.provenance import ProvenanceManager, InMemoryStorage
...
```python
from veritasreason.provenance import ProvenanceManager, SQLiteStorage
...
```python
from veritasreason.provenance import ProvenanceStorage
...
```python
from veritasreason.kg import ProvenanceTracker
...
```python
from veritasreason.split import ProvenanceTracker
from veritasreason.split.semantic_chunker import Chunk
...
```python
from veritasreason.provenance import compute_checksum, verify_checksum
...
**Important:** This module provides the *technical capabilities* for compliance. Organizations must implement additional policies, procedures, validation, and controls to meet specific regulatory requirements. VeritasReason does not provide regulatory certification or legal compliance guarantees.
