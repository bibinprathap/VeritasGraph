"""
VeritasReason Server Entry Point

This module provides the REST API server for the VeritasReason framework
...
from .core.orchestrator import VeritasReason
...
async def lifespan(app: FastAPI):
    ...
    logging.info("Starting up VeritasReason API...")
    ...
    logging.info("Shutting down VeritasReason API...")
    if getattr(app.state, "session", None) and hasattr(app.state.session.graph, "close"):
        app.state.session.graph.close()


app = FastAPI(
    title="VeritasReason API",
    description="REST API for the VeritasReason Framework",
    version=__version__,
    lifespan=lifespan,
)
...
_cors_origins_env = os.environ.get(
    "VERITASREASON_CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173"
)
...
if EXPLORER_AVAILABLE:
    ...
    logging.warning(
        ...
        "install the required dependencies: pip install 'veritas-reason[explorer]'."
    )
...
async def serve_spa(full_path: str):
    ...
    if full_path in ("", "/"):
        ...
        return JSONResponse({
            "name": "VeritasReason Knowledge Explorer",
            "version": __version__,
            "message": "Welcome to VeritasReason. The frontend is not built yet — run `npm run build` inside the explorer/ directory, or open the Vite dev server at http://localhost:5173.",
            "docs": "/docs",
            "health": "/health",
        })
    ...
    raise HTTPException(
        status_code=404,
        detail="Frontend not built. Run `npm run build` in veritasreason-explorer/ first."
    )
