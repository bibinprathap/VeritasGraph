...
Exposes VeritasReason's knowledge graph, decision intelligence, semantic extraction,
...
Environment variables:
    VERITASREASON_KG_PATH   — path to a persisted graph to load on start (optional)
    VERITASREASON_LOG_LEVEL — log level: DEBUG, INFO, WARNING (default: WARNING)
"""
...
# ── logging ────────────────────────────────────────────────────────────────
_log_level = getattr(logging, os.environ.get("VERITASREASON_LOG_LEVEL", "WARNING").upper(), logging.WARNING)
logging.basicConfig(stream=sys.stderr, level=_log_level,
                    format="%(asctime)s [veritasreason-mcp] %(levelname)s %(message)s")
log = logging.getLogger("veritasreason.mcp_server")
...
def _get_graph():
    ...
    if _graph is None:
        ...
        kg_path = os.environ.get("VERITASREASON_KG_PATH")
...
def _tool_extract_relations(args: dict) -> dict:
    ...
    if not text:
        return {"error": "text is required"}
    from veritasreason.semantic_extract import RelationExtractor, TripletExtractor
    from veritasreason.semantic_extract.cache import _result_cache
    _result_cache.clear()
...
def _tool_run_reasoning(args: dict) -> dict:
    ...
    from veritasreason.reasoning import Reasoner
    reasoner = Reasoner()
    for rule in rules:
...
def _tool_get_graph_analytics(args: dict) -> dict:
    ...
    graph = _get_graph()
    try:
        from veritasreason.kg import CentralityCalculator, CommunityDetector
...
def _tool_export_graph(args: dict) -> dict:
    ...
    graph = _get_graph()
    try:
        from veritasreason.export import RDFExporter, JSONExporter
...
TOOLS = [
    {
        ...
        "description": "Extract named entities (people, places, organisations, concepts) from text using VeritasReason NER.",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Record a decision into the VeritasReason knowledge graph with full context, causal links, and metadata.",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "add_entity",
        "description": "Add a node/entity to the VeritasReason knowledge graph.",
        "inputSchema": {
            "type": "object",
            "properties": {
...
RESOURCES = [
    {
        "uri": "veritasreason://graph/summary",
        "name": "Graph Summary",
        "description": "High-level statistics about the current knowledge graph",
        "mimeType": "application/json",
    },
    {
        "uri": "veritasreason://decisions/list",
        "name": "Decisions",
        "description": "List of all recorded decisions in the graph",
        "mimeType": "application/json",
    },
    {
        "uri": "veritasreason://schema/info",
        "name": "Schema Info",
        "description": "VeritasReason server info and available capabilities",
        "mimeType": "application/json",
    },
]
...
def _read_resource(uri: str) -> dict:
    if uri == "veritasreason://graph/summary":
        return _tool_get_graph_summary({})
    if uri == "veritasreason://decisions/list":
        return _tool_query_decisions({"limit": 50})
    if uri == "veritasreason://schema/info":
        return {
            "name": "VeritasReason",
            "version": "0.4.0",
...
def _run_stdio():
    log.info("VeritasReason MCP server starting on stdio")
    ...
    log.info("VeritasReason MCP server stopped")
