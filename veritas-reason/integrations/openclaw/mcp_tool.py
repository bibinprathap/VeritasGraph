"""
OpenClaw ↔ VeritasReason bridge
============================

Two integration paths:

1. **MCP (recommended)** — ``OpenClawMCPConfig`` emits the ``mcporter.json``
   snippet that wires VeritasReason's MCP server into the OpenClaw Gateway.
   All 12 VeritasReason MCP tools become native OpenClaw agent tools with no
   extra code.

2. **REST** — ``OpenClawKGTool`` is a plain Python class that calls the
   VeritasReason REST API (port 8000) and can be registered as an OpenClaw
...
class OpenClawMCPConfig:
    """
    Generates the ``mcporter.json`` entry needed to connect VeritasReason's MCP
    server to the OpenClaw Gateway.

    Parameters
    ----------
    server_command:
        Shell command used to launch the VeritasReason MCP server.
        Defaults to ``"python -m veritasreason.mcp_server"``.
    transport:
        MCP transport protocol.  OpenClaw supports ``"stdio"`` (default)
        and ``"sse"``.
    name:
        Key used in ``mcporter.json``.  Defaults to ``"veritasreason"``.
    ...
    def __init__(
        self,
        server_command: str = "python -m veritasreason.mcp_server",
        transport: str = "stdio",
        name: str = "veritasreason",
    ) -> None:
...
class OpenClawKGTool:
    """
    A VeritasReason knowledge-graph tool callable from an OpenClaw agent.

    Wraps the VeritasReason REST API so that an OpenClaw agent configured with
    ...
    ----------
    base_url:
        Base URL of the running VeritasReason REST server.
    ...
    dependency under ``veritas-reason[openclaw]``; install it with::

        pip install veritas-reason[openclaw]
    """

    TOOL_NAME = "veritasreason_kg"
    TOOL_DESCRIPTION = (
        "VeritasReason knowledge-graph tool. "
        ...
        if self._session is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install it with: pip install veritas-reason[openclaw]"
                ) from exc
    ...
    def run_reasoning(self, rules: List[str], facts: List[str]) -> Dict[str, Any]:
        """Run the VeritasReason forward-chaining reasoner."""
