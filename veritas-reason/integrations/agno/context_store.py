...
Implements Agno's ``MemoryDb`` protocol backed by VeritasReason's ``AgentContext``,
...
class AgnoContextStore(_MemoryDbBase):  # type: ignore[misc]
    ...
    ----------
    vector_store:
        A ``veritasreason.vector_store.VectorStore`` instance (or ``None`` to use
        an in-memory FAISS store created automatically).
    knowledge_graph:
        A ``veritasreason.context.ContextGraph`` instance (or ``None`` for a fresh
        in-memory graph).
    decision_tracking:
    ...
    def __init__(
        ...
        **agent_context_kwargs: Any,
        ...
        # ------------------------------------------------------------------
        from veritasreason.context import AgentContext, ContextGraph  # lazy import
        from veritasreason.vector_store import VectorStore  # lazy import
    ...
    # ------------------------------------------------------------------
    # Extended VeritasReason API (usable from application code directly)
    # ------------------------------------------------------------------
