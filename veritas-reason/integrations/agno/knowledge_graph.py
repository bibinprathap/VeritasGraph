"""
AgnoKnowledgeGraph — Relational agent knowledge backed by VeritasReason's KG pipeline.
...
Ingested documents pass through the full VeritasReason extraction pipeline:
...
Install
-------
    pip install veritas-reason[agno]
...
from veritasreason.utils.logging import get_logger
...
class AgnoKnowledgeGraph(_KnowledgeBase):  # type: ignore[misc]
    """
    Relational agent knowledge store backed by VeritasReason's KG pipeline.

    Parameters
    ----------
    graph_builder:
        A ``veritasreason.kg.GraphBuilder`` instance.  Created automatically if
        ``None``.
    ner_extractor:
        A ``veritasreason.semantic_extract.NERExtractor`` instance.  Created
        automatically if ``None``.
    relation_extractor:
        A ``veritasreason.semantic_extract.RelationExtractor`` instance.  Created
        automatically if ``None``.
    context_graph:
        An existing ``veritasreason.context.ContextGraph`` to use as the backing
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Lazy imports to keep veritasreason core optional at import time
