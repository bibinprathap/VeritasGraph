...
reasoning loop.  Backed by VeritasReason's ``NERExtractor``, ``RelationExtractor``,
...
class AgnoKGToolkit(_ToolkitBase):  # type: ignore[misc]
    """
    Agno Toolkit that surfaces VeritasReason's KG pipeline as agent tools.
    ...
    ner_extractor:
        A ``veritasreason.semantic_extract.NERExtractor`` instance; auto-created
        when ``None``.
    relation_extractor:
        A ``veritasreason.semantic_extract.RelationExtractor`` instance; auto-
        created when ``None``.
    reasoner:
        A ``veritasreason.reasoning.Reasoner`` instance; auto-created when
        ``None``.
    context:
    ...
    def export_subgraph(
        ...
        """
        try:
            from veritasreason.export import RDFExporter  # lazy import
