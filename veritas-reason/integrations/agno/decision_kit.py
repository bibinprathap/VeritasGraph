...
Exposes VeritasReason's decision intelligence as native Agno tools so that agents
...
class AgnoDecisionKit(_ToolkitBase):  # type: ignore[misc]
    """
    Agno Toolkit that surfaces VeritasReason's decision intelligence as agent tools.

    Parameters
    ----------
    context:
        A ``veritasreason.context.AgentContext`` (or ``AgentContext``-compatible
