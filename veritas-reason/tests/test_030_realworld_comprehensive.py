"""
Comprehensive Real-World Test Suite for VeritasReason 0.3.0-alpha and 0.3.0-beta
...
from veritasreason.context.decision_models import (
    ApprovalChain,
    PolicyException,
    Precedent,
...
class TestReasonerPatternMatching:
    ...
    def test_multi_word_value_inference(self):
        ...
        from veritasreason.reasoning.reasoner import Reasoner
        reasoner = Reasoner()
...
class TestPipelineRetryAndFailure:
    ...
    def test_failure_handler_linear_backoff(self):
        from veritasreason.pipeline.failure_handler import FailureHandler, RetryPolicy, RecoveryAction, RetryStrategy
...
# ═════════════════════════════════════════════════════════════════════════════
# 14. ProvenanceTracker – now exported from veritasreason.kg (0.3.0-beta fix)
# ═════════════════════════════════════════════════════════════════════════════

class TestProvenanceTrackerExport:
    """ProvenanceTracker must be importable from veritasreason.kg after the fix."""
...
class TestSemanticExtractRelationFix:
    ...
    def test_llm_relation_not_duplicated(self):
        ...
        from veritasreason.semantic_extract.schemas import RelationOut, RelationsResponse
        ...
        with patch("veritasreason.semantic_extract.methods.create_provider", return_value=_FakeLLM()):
