...
from veritasreason.vector_store.decision_embedding_pipeline import DecisionEmbeddingPipeline
...
class TestKGAlgorithmIntegration:
    ...
    @patch('veritasreason.vector_store.decision_embedding_pipeline.NodeEmbedder')
    ...
    @patch('veritasreason.context.context_retriever.PathFinder')
    @patch('veritasreason.context.context_retriever.CommunityDetector')
    @patch('veritasreason.context.context_retriever.CentralityCalculator')
    ...
    def test_kg_algorithm_integration_in_decision_context(self):
        ...
        from veritasreason.context import DecisionContext
        
        # Mock decision pipeline to use KG algorithms
        with patch('veritasreason.context.decision_context.DecisionEmbeddingPipeline') as mock_pipeline:
