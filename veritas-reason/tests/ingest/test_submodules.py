...
from veritasreason.ingest.api_ingestor import RESTIngestor, APIData
from veritasreason.ingest.duckdb_ingestor import DuckDBIngestor, DuckDBData
from veritasreason.ingest.elastic_ingestor import ElasticIngestor, ElasticData
from veritasreason.ingest.mcp_ingestor import MCPIngestor, MCPData
from veritasreason.ingest.mcp_client import MCPClient, MCPResource, MCPTool
from veritasreason.ingest.gdrive_ingestor import GDriveIngestor, GDriveData
from veritasreason.ingest.huggingface_ingestor import HuggingFaceIngestor, HFData
from veritasreason.ingest.mongo_ingestor import MongoIngestor, MongoData, MongoConnector
from veritasreason.ingest.pandas_ingestor import PandasIngestor, PandasData
from veritasreason.ingest.repo_ingestor import RepoIngestor, CodeFile
from veritasreason.ingest.stream_ingestor import StreamIngestor
...
class TestDuckDBIngestor:
    def test_init_raises_if_no_duckdb(self):
        ...
        with patch("veritasreason.ingest.duckdb_ingestor.duckdb", None):
    ...
    def test_ingest_csv(self):
        ...
        try:
            ...
            with patch("veritasreason.ingest.duckdb_ingestor.duckdb", mock_duckdb):
...
class TestElasticIngestor:
    def test_init_raises_if_no_elastic(self):
        with patch("veritasreason.ingest.elastic_ingestor.Elasticsearch", None):
            with pytest.raises(ImportError):
                ElasticIngestor()
    ...
    def test_ingest_index(self):
        ...
        with patch("veritasreason.ingest.elastic_ingestor.Elasticsearch", mock_es_class), \
             patch("veritasreason.ingest.elastic_ingestor.scan", mock_scan):
...
class TestMCPIngestor:
    def test_connect_and_ingest(self):
        ...
        with patch("veritasreason.ingest.mcp_ingestor.MCPClient") as MockClient, \
             patch("veritasreason.ingest.mcp_ingestor.get_progress_tracker") as mock_get_tracker:
...
class TestGDriveIngestor:
    def test_init_raises_if_no_google_libs(self):
        with patch("veritasreason.ingest.gdrive_ingestor.build", None):
            with pytest.raises(ImportError):
                GDriveIngestor()
    ...
    def test_ingest_folder(self):
        ...
        # Mock downloader
        with patch("veritasreason.ingest.gdrive_ingestor.MediaIoBaseDownload") as MockDownloader, \
             patch("veritasreason.ingest.gdrive_ingestor.build") as mock_build, \
             patch("veritasreason.ingest.gdrive_ingestor.InstalledAppFlow"), \
             patch("veritasreason.ingest.gdrive_ingestor.Credentials"):
...
class TestHuggingFaceIngestor:
    def test_init_raises_if_no_datasets(self):
        with patch("veritasreason.ingest.huggingface_ingestor.load_dataset", None):
            with pytest.raises(ImportError):
                HuggingFaceIngestor()

    def test_ingest_dataset(self):
        with patch("veritasreason.ingest.huggingface_ingestor.load_dataset") as mock_load:
            # Mock dataset
            mock_data = [
                {"col1": "val1", "col2": 1},
                {"col1": "val2", "col2": 2}
...
class TestMongoIngestor:
    def test_init_raises_if_no_pymongo(self):
        with patch("veritasreason.ingest.mongo_ingestor.MongoClient", None):
            with pytest.raises(ImportError):
                MongoIngestor()

    def test_ingest_collection(self):
        with patch("veritasreason.ingest.mongo_ingestor.MongoClient") as MockClient:
            mock_client = MockClient.return_value
            mock_db = MagicMock()
            mock_coll = MagicMock()
...
class TestRepoIngestor:
    def test_ingest_repository(self):
        ...
        try:
            ...
            with patch("veritasreason.ingest.repo_ingestor.git.Repo") as MockRepo, \
                 patch("veritasreason.ingest.repo_ingestor.tempfile.mkdtemp") as mock_mkdtemp, \
                 patch("veritasreason.ingest.repo_ingestor.shutil.rmtree"), \
                 patch("veritasreason.ingest.repo_ingestor.get_progress_tracker") as mock_get_tracker:
...
class TestStreamIngestor:
    def test_ingest_kafka(self):
        with patch("veritasreason.ingest.stream_ingestor.KafkaProcessor") as MockProcessor:
