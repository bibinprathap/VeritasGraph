...
from veritasreason.split.splitter import TextSplitter
from veritasreason.split.semantic_chunker import SemanticChunker, Chunk

class TestSplitter(unittest.TestCase):

    def setUp(self):
        self.mock_logger = MagicMock()
        self.logger_patcher = patch('veritasreason.split.splitter.get_logger', return_value=self.mock_logger)
        self.logger_patcher_sc = patch('veritasreason.split.semantic_chunker.get_logger', return_value=self.mock_logger)
        self.tracker_patcher = patch('veritasreason.split.semantic_chunker.get_progress_tracker', return_value=MagicMock())
    ...
    @patch('veritasreason.split.semantic_chunker.spacy')
