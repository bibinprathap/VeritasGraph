"""
VeritasReason MCP Server — JSON-RPC 2.0 over stdio.
...
can interact with the VeritasReason knowledge graph.
...
log = logging.getLogger("veritasreason.mcp.server")
...
class VeritasReasonMCPServer:
    """VeritasReason MCP server — reads JSON-RPC requests from stdin, writes to stdout."""
    ...
    def run(self) -> None:
        ...
        log.info("VeritasReason MCP server starting (stdio)")
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="VeritasReason MCP Server")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    args = parser.parse_args()
    VeritasReasonMCPServer(debug=args.debug).run()
